...
$stmt = $conn->prepare($query_user_check);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$stmt->store_result();

// Jika user tidak ada, logout dan arahkan ke halaman login
if ($stmt->num_rows === 0) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_task'])) {
    ...
    if (!empty($nama) && !empty($prioritas) && !empty($tanggal)) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssss", $nama, $prioritas, $tanggal, $user_id);
            if ($stmt->execute()) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_subtask'])) {
    ...
    if (!empty($task_id) && !empty($subtask_nama)) {
        ...
        // Prepared Statement untuk menghindari SQL Injection
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("is", $task_id, $subtask_nama);
            if ($stmt->execute()) {
                header("Location: index.php");
                exit();
            } else {
    ...
    <ul>
        <?php while ($row = $result->fetch_assoc()): ?>
            <li class="task-item" data-deadline="<?= $row['tanggal']; ?>" data-name="<?= $row['nama']; ?>">
                ...
                <a href="edit.php?id=<?= $row['id']; ?>" class="btn-delete">Edit</a>
